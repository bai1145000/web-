...
from orders.models import  OrdersMessage, OrderGoods
...
class OrderCommitView(View):
    ...
    def post(self,request):
        ...
        pay_method = request.POST.get('pay_method') #付款方式
        ...
        try:
            ...
            order =  OrdersMessage.objects.create(order_id=order_id,
                                             user=user,
                                             addr= addr,
...
class OrderPayView(View):
    ...
    def post(self, request):
        ...
        try:
            order =  OrdersMessage.objects.get(order_id=order_id,
                                          user=user,
                                          pay_method=3,
                                          order_status=1)
        except  OrdersMessage.DoesNotExist:
            return JsonResponse({'res':2, 'errmsg':'订单错误'})

        # 业务处理:使用python sdk调用支付宝的支付接口
        # 初始化
        alipay = AliPay(
            appid="2088621957630142", # 应用id
...
class CommentView(LoginRequiredMixin, View):
    ...
    def get(self, request, order_id):
        ...
        try:
            order =  OrdersMessage.objects.get(order_id=order_id, user=user)
        except  OrdersMessage.DoesNotExist:
            return redirect(reverse("user:order"))

        # 根据订单的状态获取订单的状态标题
        order.status_name =  OrdersMessage.ORDER_STATUS[order.order_status]
